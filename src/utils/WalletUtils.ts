...
export class WalletUtils {
    ...
    public static verifyAddressRecord(record: AddrRecord): WalletVerificationCodes {
        ...
        try {
            ...
            signerBtcAddr = EthereumUtils.recoverSignerBitcoinAddress(record);
        ...
        return (signerEthAddr == JSON.parse(record.data).addr ||
                signerBtcAddr == JSON.parse(record.data).addr)
    ...
    public static validateWallets(key: string, val: any, baseID: string): WalletVerificationStatus {
        ...
        if (key != WalletManagerImpl.DATA_KEY_WALLETS) {
            result.err = 'The \<key\> is expected to be "' + WalletManagerImpl.DATA_KEY_WALLETS + '"';
    ...
    public static createAddersRecord(baseID: string, addr: string, ethPrvKey: string): AddrRecord {
