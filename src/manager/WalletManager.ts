...
export class WalletManager {
    ...
    public async addWealthValidator(validatorPbKey: string) {
        // Alice adds wealth record pointing to Validator's
        const myData: Map<string, string> = await this.profileManager.getData();
        myData.set(WalletManager.DATA_KEY_ETH_WEALTH_VALIDATOR, validatorPbKey);
    ...
    public async refreshWealthPtr(): Promise<WealthPtr> {
        ...
        if (data.has(WalletManager.DATA_KEY_WEALTH)) {
            const wealth: string = data.get(WalletManager.DATA_KEY_WEALTH) || '';
            wealthPtr = Object.assign(new WealthPtr(), JSON.parse(wealth));

        } else if (data.has(WalletManager.DATA_KEY_ETH_WEALTH_VALIDATOR)) {
            const validatorPbKey: string = data.get(WalletManager.DATA_KEY_ETH_WEALTH_VALIDATOR) || '';
            ...
            if (recordsFromValidator.length > 0) {
                ...
                data.set(WalletManager.DATA_KEY_WEALTH, JSON.stringify(wealthPtr));
            ...
            throw WalletManager.DATA_KEY_ETH_WEALTH_VALIDATOR + ' data not exist!';
        }
