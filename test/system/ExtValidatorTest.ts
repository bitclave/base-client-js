...
describe('BASE API test: External Validator', async () => {
    ...
    it('Alice grants access to validator', async () => {
        ...
        await baseAlice.profileManager.updateData(
            new Map([[WalletManagerImpl.DATA_KEY_WALLETS, 'test eth wallets']])
        );

        const grantFields: Map<string, AccessRight> = new Map();
        grantFields.set(WalletManagerImpl.DATA_KEY_WALLETS, AccessRight.R);
        ...
        decryptedObj.get(WalletManagerImpl.DATA_KEY_WALLETS).should.be.equal('test eth wallets');
    });
    ...
    it('Validator asks Alice for access', async () => {
        ...
        // Validator asks Alice to get access to wallets
        const id: number = await baseValidator.dataRequestManager.requestPermissions(
            accAlice.publicKey,
            [WalletManagerImpl.DATA_KEY_WALLETS]
        );
    ...
    it.only('full flow with Alice, Bob, Carol, Desearch and Validator', async () => {
        // to get this records, I used example application, created a wallet records and did copy&paste
        try {
            var wallets = [
                '{"data":[{"data":"{\\"baseID\\":\\"02fe55f705abc712a8a80c43442ebe19ab5e66e5b165a0619440438385bc08383a\\",\\"addr\\":\\"0xa5f79860b160dee0943bd1f6a5713a61dc56f931\\"}","sig":"0x5227af80548be15c9d5f5d66fcf8459df7e4203bf4508497133c9e2054a6ed281dfbbc5fdb7f44801fe93da514613efba96efc8e52209c74bb2d1e1769f8ebba1b"}],"sig":"IIBNKCKMHVBq6LpneRJi9ZNia7U5KCHfJq4TotM65xmYSZ7uGH5V2onnvOwK+1/EZQEg6dRzuYgp3PTVRShA4Nc="}',
                '{"data":[{"data":"{\\"baseID\\":\\"0303247051dc50d32aaacd7b36c96792a8950a0fda1fdf5a41231e67224865f61f\\",\\"addr\\":\\"0xa5f79860b160dee0943bd1f6a5713a61dc56f931\\"}","sig":"0x55b61cea89a84722288807553ad35994bdb45813a6273dee35d5b33bc49cfaa879d8b2c5451ec191cdc6974ed3ad7e3050130d4498915134debb2ff977dd83e91c"}],"sig":"INzbg3aOjA4qc+PD9K787CEFUORJGw5iv/09rEZGR3mSdmaWpDe9OfLSLKBTbk9BI6ZCzm/k5To8/Sw4QrjV07k="}',
                '{"data":[{"data":"{\\"baseID\\":\\"02de5804b34c2f3b3d917de52c4768984e9200e799df9bd012cfd03fe428d8fafd\\",\\"addr\\":\\"0xa5f79860b160dee0943bd1f6a5713a61dc56f931\\"}","sig":"0x8ee4014de7b828adeede57f1abfa626d058f52881df4268d67439a11bcbe03f16aaa8e70e7f7f6bff2b7f8bb2714b4b0002e959be73f3f191e42a4b2f04668f31b"}],"sig":"IMmdINnrSQS6VaQOXqr2/BX/LlIFl34stcEOfl6kR9HRJl8BkkNbDWzYm+xI0jA6ZAdKT70TTXO6OX4ST8n2cHo="}'
            ];
            var accs = [
            ...
            await baseAlice.profileManager.updateData(new Map([[WalletManagerImpl.DATA_KEY_WALLETS, wallets[0]]]));
            await baseAlice.walletManager.addWealthValidator(accValidator.publicKey);

            // create wallets for Bob and grantt access for Validator
            await baseBob.profileManager.updateData(new Map([[WalletManagerImpl.DATA_KEY_WALLETS, wallets[1]]]));
            ...
            await baseCarol.profileManager.updateData(new Map([[WalletManagerImpl.DATA_KEY_WALLETS, wallets[2]]]));
            ...
            for (var i = 0; i < requestsByFrom.length; i++) {
                ...
                decryptedObj.get(WalletManagerImpl.DATA_KEY_WALLETS).should.be.equal(wallets[i]);
